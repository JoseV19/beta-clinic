...
function monthYearLabel(dates: string[]) {
  ...
  const fmt = (d: Date) =>
    d.toLocaleDateString('es-GT', { month: 'long', year: 'numeric' })
  if (first.getMonth() === last.getMonth()) return fmt(first)
  return `${first.toLocaleDateString('es-GT', { month: 'short' })} – ${fmt(last)}`
}

const estadoColor: Record<string, string> = {
  confirmada: 'bg-beta-mint/20 text-emerald-800 dark:text-beta-mint',
  pendiente: 'bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-400',
  completada: 'bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-400',
  cancelada: 'bg-red-100 text-red-700 dark:bg-red-500/15 dark:text-red-400',
  no_show: 'bg-gray-100 text-gray-600 dark:bg-clinical-white/10 dark:text-clinical-white/60',
}
...
export default function Agenda() {
  ...
  const [editingAppt, setEditingAppt] = useState<AgendaAppointment | undefined>()
  const [slotDate, setSlotDate] = useState<string | undefined>()
  const [slotHora, setSlotHora] = useState<string | undefined>()

  // Filter appointments for current week
  const weekAppts = useMemo(
    () => appointments.filter((a) => dates.includes(a.fecha)),
    [appointments, dates],
  )
  ...
  function handleCellClick(date: string, hour: number) {
    setEditingAppt(undefined)
    setSlotDate(date)
    setSlotHora(`${hour.toString().padStart(2, '0')}:00`)
    setModalOpen(true)
  }

  function handleApptClick(a: AgendaAppointment) {
    setEditingAppt(a)
    setSlotDate(undefined)
    setSlotHora(undefined)
    setModalOpen(true)
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {view === 'calendar' && (
        <CalendarWidget appointments={appointments} onSelectAppointment={handleApptClick} />
      )}
                ...
                <tr key={hour} className="group">
                  ...
                  {dates.map((date) => {
                    ...
                    return (
                      <td
                        key={key}
                        onClick={() => items.length === 0 && handleCellClick(date, hour)}
                        className="h-16 border-b border-l border-omega-violet/5 p-1 align-top transition-colors hover:bg-omega-violet/[0.02] dark:border-clinical-white/5 dark:hover:bg-clinical-white/5 cursor-pointer"
                      >
                        {items.map((a) => (
                          <div
                            key={a.id}
                            onClick={(e) => { e.stopPropagation(); handleApptClick(a) }}
                            className={`group/card relative mb-1 cursor-pointer rounded-md px-1.5 py-1 leading-tight ${
                              estadoColor[a.estado] ?? estadoColor.pendiente
                            }`}
                          >
                            <span className="block truncate pr-1 font-semibold">
                              {a.patientName}
                            </span>
